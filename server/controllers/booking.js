...
exports.postBooking = async (req, res) => {
  ...
  await booking.save();
  await bus.save();
  //implement a payment system here ,so that when the user has payed we change the payment status to paid 
  //and also the booking status will change accordinly ,if not we send an email that they should complete their
  // payment in 5 minutes otherwise we will refresh the booking status to null
  // email data
